...
// Deterministic elevation stand-in per story (for visual variety) so
// covers aren't all the same gradient -- stories don't carry real
// elevation data the way Experiences do, so we derive a stable pseudo
// value from the slug instead.
function pseudoElevation(slug: string) {
	const hash = [...slug].reduce((acc, c) => acc + c.charCodeAt(0), 0);
	const bands = [3200, 900, 120, -50, -400];
...
export function StoryCard({ story, large = false }: { story: Story; large?: boolean }) {
	return (
		...
		>
			<div
				className={`relative ${large ? "aspect-[16/9]" : "aspect-[4/3]"} overflow-hidden`}
				style={{
			...
			<div className="flex flex-1 flex-col p-5">
				<h3 className={`font-display font-semibold leading-snug text-ink ${large ? "text-2xl" : "text-lg"}`}>{story.title}</h3>
